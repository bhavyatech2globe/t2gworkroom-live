...
class Hr_profile_model extends App_Model
{
	...
	public function add_staff($data)
	{
		...
		if (isset($data['dob_original'])) {
			$data['dob_original'] = to_sql_date($data['dob_original']);
		} else {
			$data['dob_original'] = null;
		}
		if (isset($data['doj'])) {
		...
		// print_r($data); die;
		// $sql = $this->db->set($data)->get_compiled_insert(db_prefix() . 'staff');
		// echo $sql; die;
	...
	public function update_staff($data, $id)
	{
		...
		// if ($_FILES && $_FILES['identification']['name']) {
		// 	$data['identification'] = $this->onboarding_model->upload_onboarding_data($id, 'identification');
		// }

		// if ($_FILES && $_FILES['_10_marksheet']['name']) {

		// 	$data['_10_marksheet'] = $this->onboarding_model->upload_onboarding_data($id, '_10_marksheet');
		// }
		// if ($_FILES && $_FILES['_12_marksheet']['name']) {

		// 	$data['_12_marksheet'] = $this->onboarding_model->upload_onboarding_data($id, '_12_marksheet');
		// }


		// if ($_FILES && $_FILES['graduation_marksheet']['name']) {
		// 	$data['graduation_marksheet'] = $this->onboarding_model->upload_onboarding_data($id, 'graduation_marksheet');
		// }

		// if ($_FILES && $_FILES['post_graduation_marksheet']['name']) {

		// 	$data['post_graduation_marksheet'] = $this->onboarding_model->upload_onboarding_data($id, 'post_graduation_marksheet');
		// }
		// if ($_FILES && $_FILES['previous_uploads']['name']) {

		// 	$data['previous_uploads'] = $this->onboarding_model->upload_onboarding_data($id, 'previous_uploads');
		// }
		// if ($_FILES && $_FILES['before_uploads']['name']) {

		// 	$data['before_uploads'] = $this->onboarding_model->upload_onboarding_data($id, 'before_uploads');
		// }

		// if ($_FILES && $_FILES['declaration_signature']['name']) {
		// 	$data['declaration_signature'] = $this->onboarding_model->upload_onboarding_data($id, 'declaration_signature');
		// }

		// if ($_FILES && $_FILES['agreement_signature']['name']) {
		// 	$data['agreement_signature'] = $this->onboarding_model->upload_onboarding_data($id, 'agreement_signature');
		// }
		...
		if ($affectedRows > 0) {
			...
			$this->send_onboarding_email_to_hr($data);
			return true;
		}
	...
	// custom function to send email to HR when form is filled

	public function send_onboarding_email_to_hr($data)
	{
		...
		$subject = 'HR has updated a new record for EMP ID - ' . $data['staff_identifi'];
        ...
        <head>
            ...
            <title> HR has updated a new record</title>
        </head>
        <body>
            <p>HR has updated the Record of Employee - <b>' . $data['firstname'] . ' ' . $data['lastname'] . '</b></p>
	...
	public function update_staff_info_to_staff($data)
	{

		$this->db->set(array(
			'staff_identifi' => $data['empid'],
			'doj' => date('Y-m-d', strtotime($data['doj']))
		));
		$this->db->where('staffid', $data['staffid']); // Add your condition here
		$this->db->update('tblstaff');

		// echo $query; die;
		if ($this->db->update('tblstaff')) {
			return true;
		}
		return false;
	}
