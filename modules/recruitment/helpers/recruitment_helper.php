...
function get_candidate_cv($id)
{
    $CI = &get_instance();
    $CI->db->where('id', $id);
    $candidate = $CI->db->get(db_prefix() . 'rec_candidate')->row();
    if ($candidate && $candidate->web_view_link != '') {
        return $candidate->web_view_link;
    } else {
        return '';
    }
}

function get_candidate_hr_remarks($id)
{
    $CI = &get_instance();
    $CI->db->where('id', $id);
    $candidate = $CI->db->get(db_prefix() . 'rec_candidate')->row();
    if ($candidate && $candidate->remarks != '') {
...
function get_candidate_scheduled_by($id)
{
    $CI = &get_instance();
    $CI->db->where('id', $id);
    $candidate = $CI->db->get(db_prefix() . 'rec_interview')->row();
    if ($candidate && $candidate->added_from != '') {
        return get_staff_full_name($candidate->added_from);
    } else {
        return '';
    }
}

function check_candidate_added_by($candidate_id, $staff_id = '')
{
    if ($staff_id == '') {
        $staff_id = get_staff_user_id();
    }
    $CI = &get_instance();
    $CI->db->where('added_from', $staff_id);
