...
if (is_admin() || in_the_department('HR')) {
    $where = [];
} else {
    $where = [" WHERE FIND_IN_SET(" . get_staff_user_id() . ",interviewer) > 0"];
...
foreach ($rResult as $aRow) {
    ...
    for ($i = 0; $i < count($aColumns); $i++) {
        ...
        if ($aColumns[$i] == 'tblrec_interview.added_from') {
            ...
            $_data = '<a href="' . admin_url('recruitment/feedback_form?candidate=' . $aRow['candidate'] . '&round=' . $aRow['round']) . '"><i class="fa-solid fa-pen-to-square"></i></a>';
            ...
            $_data = '<a href="' . admin_url('recruitment/candidate/' . $aRow['candidate']) . '">' . get_candidate_name($aRow['candidate']) . '</a>';
