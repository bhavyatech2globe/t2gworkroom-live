...
class recruitment extends AdminController
{
	...
	public function candidate_profile()
	{
		...
		$data['total_candidates'] = $this->db->query("select count(*) as total from tblrec_candidate")->row()->total;
		$data['todays_candidates'] = $this->db->query("SELECT count(*) as total FROM `tblrec_candidate` WHERE date_add = CURDATE();")->row()->total;
		$data['yesterday_candidates'] = $this->db->query("SELECT count(*) as total FROM `tblrec_candidate` WHERE date_add = CURDATE() - INTERVAL 1 DAY;")->row()->total;
		$data['weekly_candidates'] = $this->db->query("SELECT count(*) as total FROM `tblrec_candidate` WHERE date_add >= CURDATE() - INTERVAL ( WEEKDAY(CURDATE()) + 1 ) DAY AND date_add <= CURDATE();")->row()->total;
	...
	public function add_update_candidate($id = '')
	{
		...
		if ($data) {
			...
			if ($id == '') {
				...
				$uploaded = $this->upload_file_to_gdrive();
				...
				// print_r($data);
				// die;
				// $data['file_id'] = $this->upload_cv_google_drive();
	...
	public function get_candidate_infor_change($candidate)
	{
		...
		echo json_encode([
			...
			'job_position' => $infor->job_position,

		]);
	}


	public function get_candidate_infor_change_edit($candidate, $round)
	{
		$infor = $this->recruitment_model->get_candidates($candidate);

		$interview_data = $this->db->query("select * from tblrec_interview where candidate = $candidate and round = $round")->row();
		...
		echo json_encode([
			...
			'phonenumber' => $infor->phonenumber,
			'job_position' => $infor->job_position,
			'rec_campaign' => $infor->rec_campaign,
			'interview_date' => $interview_data->interview_day,
			'from' => $interview_data->from_time,
			'to' => $interview_data->to_time,
			'round' => $interview_data->round,
			'interview_link' => $interview_data->interview_link,
			'interviewers' => $interview_data->interviewer,
	...
	public function interview_schedules()
	{
		if ($this->input->post()) {
			...
			// echo '<pre>';
			// print_r($data);die;
			$data['scheduled_by'] = get_staff_user_id();

			if (!$this->input->post('id')) {
				$id = $this->recruitment_model->add_interview_schedules($data);
				$this->recruitment_model->send_interview_email($data);
				if ($id) {
	...
	/**
	 * custom interviewer table BY bhavya
	 * @return
	 */
	public function interviewer_table()
	{
		if ($this->input->is_ajax_request()) {
			$this->app->get_table_data(module_views_path('recruitment', 'interview_schedule/interviewer_table'));
		}
	}
	...
	public function get_candidate_sample()
	{
		if ($this->input->is_ajax_request()) {
			...
			$cd = $this->recruitment_model->get_job_position();
			$html = '';
			$total_candidate = 1;
			...
			$html .= '<div class="row col-md-12" id="candidates-item">
    		<div class="col-md-4 form-group">
    		<select id="position" class="selectpicker" disabled="true" data-live-search="true" data-width="100%" data-none-selected-text="' . _l('ticket_settings_none_assigned') . '" required>
    		<option value=""></option>';
			foreach ($cd as $s) {
				$attr = '';
				$html .= '<option value="' . $s['position_id'] . '" ' . $attr . ' >' . $s['position_name'] . '</option>';
			}
			$html .= '</select>
    		</div>
			<input type="hidden" id="hiddenSelect" name="position">
    		<div class="col-md-4">
	...
	public function view_interviews($id = '')
	{
		$data['staffs'] = $this->staff_model->get();
		$data['candidates'] = $this->recruitment_model->get_candidates();
		$data['list_cd'] = $this->recruitment_model->get_list_cd();
		...
		$this->load->view('interview_schedule/view_interviews', $data);
	}
	...
	public function feedback_form()
	{
		...
		// echo 'value : ' . check_staff_is_interviewer() || is_admin();
		// die;

		if (check_staff_is_interviewer() || is_admin()) {

			if ($this->input->get('candidate')) {
				$id = $this->input->get('candidate');
				$round = $this->input->get('round');
				$data['candidate_id'] = $id;
				$data['candidate_name'] = get_candidate_name($id);
				$position_id = $this->recruitment_model->get_candidate_position($id);
				$data['position'] = get_rec_position_name($position_id);
				$data['round'] = $round;
				$data['title'] = 'Interview Feedback Form';

				$data['previous_rounds_data'] = $this->recruitment_model->get_all_interview_data($id);

				$this->load->view('interview_schedule/feedback_form', $data);
			} else {
				access_denied();
			}
		} else {
	...
	public function add_feedback()
	{
		...
		$data['remarks'] = $this->security->xss_clean($data['remarks']);


		$this->recruitment_model->insert_update_feedback($data);

		redirect(admin_url('recruitment/feedback_form?candidate=' . $data['candidate'] . '&round=' . $data['round']));
	}


	public function upload_cv_google_drive()
	{
		$client = new Google_Client();
		$client->setAuthConfig($this->config->item('service_account_credentials_json'));
		...
		$fileMetadata = new Google_Service_Drive_DriveFile(
			array(
				'name' => $file['name'],
				'parents' => array($this->config->item('folder_id'))
			)
		);
	...
	{
		if (isset($_FILES['cv_link']['name'])) {
			// Load the Google Drive service
			// $service = new Google_Service_Drive($this->client);

			// Create a new folder

			$folderNname = Date('Y-m-d');
			$folderId = $this->getFolderId($folderNname);
			// Upload the file
			$file = new Google_Service_Drive_DriveFile();
			$file->setName($_FILES['cv_link']['name']);
			$file->setParents([$folderId]);

			$result = $this->service->files->create($file, [
				'data' => file_get_contents($_FILES['cv_link']['tmp_name']),
				'mimeType' => $_FILES['cv_link']['type'],
				'uploadType' => 'multipart',
	...
	public function create_gdrive_folder()
	{
		...
		$folderMetadata = new Google_Service_Drive_DriveFile([
			...
			'parents' => ['1X8igwsOqOPkPP9fgORKicpCIhYqmBbUZ']
		]);
