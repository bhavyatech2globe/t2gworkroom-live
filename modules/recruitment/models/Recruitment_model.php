...
class Recruitment_model extends App_Model
{
	...
	public function add_recruitment_campaign($data)
	{
		...
		$data['cp_position'] = implode(',', $data['cp_position']);
	...
	public function add_candidate($data)
	{
		...
		/* new added from column created in  */
		...
		/* added the custom field for the cv link */
		if (isset($data['cv_link'])) {
			$data['cv_link'] = htmlspecialchars($data['cv_link'], ENT_QUOTES, 'UTF-8');
			// unset($data['cv_link']);
		}
	...
	public function add_interview_schedules($data)
	{
		$no_of_interviews = count($data['interview_day']);

		if ($no_of_interviews > 0) {
			for ($i = 0; $i < $no_of_interviews; $i++) {

				$processed_data['campaign'] = $data['campaign'];
				$processed_data['position'] = $data['position'];

				$processed_data['interview_day'] = $data['interview_day'][$i];
				if (!$this->check_format_date($processed_data['interview_day'])) {
					$processed_data['interview_day'] = to_sql_date($processed_data['interview_day']);
				}
				$processed_data['interviewer'] = implode(',', $data['interviewer'][$i]);
				$processed_data['added_from'] = get_staff_user_id();
				$processed_data['added_date'] = date('Y-m-d');

				$processed_data['from_time'] = $data['from_time'][$i];
				$processed_data['to_time'] = $data['to_time'][$i];

				$processed_data['from_hours'] = ($processed_data['interview_day'] . ' ' . $processed_data['from_time'] . ':00');
				$processed_data['to_hours'] = $processed_data['interview_day'] . ' ' . $processed_data['to_time'] . ':00';

				$processed_data['round'] = $data['round'][$i];

				if (!$this->check_format_date($processed_data['interview_day'])) {
					$processed_data['from_hours'] = to_sql_date($processed_data['interview_day']) . ' ' . $data['from_time'][$i] . ':00';
				}

				if (!$this->check_format_date($data['interview_day'][$i])) {
					$processed_data['to_hours'] = to_sql_date($processed_data['interview_day']) . ' ' . $data['to_time'][$i] . ':00';
				}

				$processed_data['interview_link'] = $data['interview_link'][$i];
				$processed_data['candidate'] = $data['candidate'];


				$this->db->insert(db_prefix() . 'rec_interview', $processed_data);

				if ($this->db->affected_rows() > 0) {

					foreach ($data['interviewer'][$i] as $interviewer) {

						add_notification([
							'fromuserid' => get_staff_user_id(),
							'description' => 'You have Interview Scheduled',
							'link' => 'recruitment/view_interviews',
							'touserid' => $interviewer,
						]);
					}
				}
	...
	public function update_interview_schedules($data, $id)
	{
		...
		if ($no_of_interviews > 0) {
			for ($i = 0; $i < $no_of_interviews; $i++) {
				...
				if ($i == 0) {
					$this->db->where('id', $id);
					$this->db->update(db_prefix() . 'rec_interview', $processed_data);
				} else {
					$this->db->insert(db_prefix() . 'rec_interview', $processed_data);
				}
	...
	{
		$check_query = "SELECT * FROM tblrec_interview_feedback WHERE candidate = ? AND round = ? AND added_from = ?";
		$update_array = array('candidate' => $data['candidate'], 'round' => $data['round'], 'added_from' => $data['added_from']);
		$is_data_present = $this->db->query($check_query, $update_array)->result_array();
	...
	{
		$query = $this->db->query("SELECT * FROM tblrec_interview_feedback WHERE candidate = ? ", $candidate_id);
		$data = $query->result_array();
		return $data;
	}

	public function send_interview_email($data)
	{
		...
		$all_interviewers = $data['interviewer'];

		// echo count($all_interviewers);die;
		// for ($i = 0; $i < count($all_interviewers); $i++) {

		// 	foreach ($data['interviewer'][$i] as $interviewer) {
		$i = 0;
		foreach ($all_interviewers as $subArray) {
			// foreach ($subArray as $ids) {
			foreach ($subArray as $interviewer) {

				$interviewer_email = get_staff_email_id($interviewer);
				$startTime = date('Y-m-d H:i:s', strtotime($data['interview_day'][$i] . ' ' . $data['from_time'][$i]));
				$endTime = date('Y-m-d H:i:s', strtotime($data['interview_day'][$i] . ' ' . $data['to_time'][$i]));
				$position = get_rec_position_name($data['position']);
				$scheduled_by = get_staff_full_name($data['scheduled_by']);
				$description = get_candidate_hr_remarks($data['candidate']);
				$attendee = get_staff_full_name($interviewer);
				$eventDetails = [
					'start_time' => $startTime,
					'end_time' => $endTime,
					'summary' => 'Interview is Scheduled.',
					'description' => 'Please take the interview for the candidate',
					'location' => 'Tech2Globe',
					'attendee_name' => $attendee,
					'attendee_email' => $interviewer_email,
					'organizer_name' => 'T2G Workroom',
					'organizer_email' => 'noreply@t2gworkroom.com'
				];
				...
				$this->email->from($eventDetails['organizer_email'], $eventDetails['organizer_name']);

				$this->email->to(get_staff_email_id($interviewer));
				// $email_to = 'bhavya.khanna@tech2globe.in';
				// $this->email->to(['bhavyakhanna.tech2globe@gmail.com', 'bhavya.khanna@tech2globe.in']);
				$this->email->subject(  get_candidate_name($data['candidate']) . ' | ' . get_rec_position_name($data['position']) . ' | ' . get_staff_full_name($interviewer) . ' | Round ' . $data['round'][$i] );



				$icsContent = $this->createICS($eventDetails);
				...
				<body>
					<p> Hello ' . $attendee . ' ,  <br><br>
					<b>Interview Date :</b> ' . $data['interview_day'][$i] . '<br>
					<b>Interview Time :</b> ' . $data['from_time'][$i] . ' to ' . $data['to_time'][$i] . '<br>
					<b>Position :</b> ' . $position . '<br>
					<b>CV Link :</b> <a href = "' . get_candidate_cv($data['candidate']) . '">Click Here</a><br>
					<b>Scheduled By :</b> ' . $scheduled_by . '<br>
					<b>HR Remarks :</b> ' . $description . ' <br><br>
					
					<b>Interview Link:</b> <a href = "' . $data['interview_link'][$i] . '">Click Here</a> <br>
					<em>NOTE: Kindly share your remarks of the interview using Workroom. Don\'t respond on this email.</em></p>
				</body>
				</html>
				';

				// Attach the iCalendar file
				$this->email->attach($icsContent, 'attachment', 'invite.ics', 'text/calendar; charset=utf-8; method=REQUEST');
				...
				// Optional: Delay between sends to avoid hitting rate limits
	...
	function createICS($eventDetails)
	{
		...
		$ics .= "DTSTART:" . gmdate('Ymd\THis\Z', strtotime($eventDetails['start_time'])) . "\r\n";
		$ics .= "DTEND:" . gmdate('Ymd\THis\Z', strtotime($eventDetails['end_time'])) . "\r\n";
		$ics .= "SUMMARY:" . $eventDetails['summary'] . "\r\n";
		$ics .= "DESCRIPTION:" . $eventDetails['description'] . "\r\n";
		$ics .= "LOCATION:" . $eventDetails['location'] . "\r\n";
		$ics .= "ORGANIZER;CN=" . $eventDetails['organizer_name'] . ":MAILTO:" . $eventDetails['organizer_email'] . "\r\n";
		$ics .= "ATTENDEE;CN=" . $eventDetails['attendee_name'] . ";RSVP=TRUE:MAILTO:" . $eventDetails['attendee_email'] . "\r\n";
		$ics .= "END:VEVENT\r\n";
		$ics .= "END:VCALENDAR\r\n";
