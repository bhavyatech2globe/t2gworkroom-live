      ...
      var calendar_settings = {
        ...
        moreLinkClick: function (info) {
           ...
           eventDidMount: function (data) {
				...
				$el.attr('status', data.event.extendedProps._status);
                $el.attr('onclick', data.event.extendedProps.onclick);
                $el.attr('data-toggle', 'tooltip').tooltip({html:true});;
  ...
  initDataTable(table_registration_leave, admin_url+'timesheets/table_registration_leave', [1], [1], requisitionServerParams, [1, 'desc']);
    ...
    $('input[name="start_time_s"]').change(function(){
      start_time_check(this,'input[name="end_time_s"]');
	   start_type_check(this,'#type');
	   start_type_check_type(this,'#type_check');
	   start_type_check_date(this,'#date');
    ...
    $('input[name="start_time"]').change(function(){
	  ...
	  var valid = start_type_check(this,'#type');
	  var valid = start_type_check_type(this,'#type_check');
	  var valid = start_type_check_date(this,'#date');
      if(valid){
        get_day_from_date();
      }
    });
...
function get_day_from_date(){
    ...
    $.post(admin_url + 'timesheets/calculate_number_days_off',data).done(function(response){
     ...
     $('input[name="number_of_leaving_day"]').val(response);
	  //var type = $('#type_check_in').text(response[0].type_check);
	  if(response[0].type_check == "1"){
		var type =  $('#type_check_in').text('Check In Time');
	  }
	  $('#type_check_date_in').text(response[0].date);
	  
	  
	  if(response[1].type_check == "2"){
		 var type =  $('#type_check_out').text('Check Out Time');
	  }
	  $('#type_check_date_out').text(response[1].date);
