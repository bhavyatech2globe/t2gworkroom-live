...
class timesheets extends AdminController
{
	...
	public function data_check()
	{
		...
		$orgDate = $start_time;
		$start_date = date("Y-m-d", strtotime($orgDate));
		$end_time = $_GET['end_time'];
		$orgDate1 = $end_time;
		$end_date = date("Y-m-d", strtotime($orgDate1));
		$type = $_GET['value'];
		//echo 'SELECT * FROM tbltimesheets_requisition_leave WHERE date(start_time) BETWEEN "'.$start_date.'" AND "'.$end_date.'" AND date(end_time) BETWEEN "'.$start_date.'" AND "'.$end_date.'" AND staff_id = "'.$staff_id.'"';die;	
		$res  = $this->db->query('SELECT * FROM tbltimesheets_requisition_leave WHERE date(start_time) BETWEEN "' . $start_date . '" AND "' . $end_date . '" AND date(end_time) BETWEEN "' . $start_date . '" AND "' . $end_date . '" AND staff_id = "' . $staff_id . '"')->result_array();
		//print_r($res);die;
		$count  = count($res);
		...
		if ($count > 0) {
			foreach ($res as $result) {
				...
				$start_date_db = date("Y-m-d", strtotime($start));
				$end = $result['end_time'];
				$end_date_db = date("Y-m-d", strtotime($end));
				$current_date = date("Y-m-d");
				if (($end_date == $end_date_db)) {
					echo 1;
				} else if ($start_date == $start_date_db and $current_date != $start_date_db) {
					echo 1;
				} else {
					echo 0;
				}
	...
	public function leave_check()
	{
		...
		$type = $_GET['value'];
		$this->db->select('*');
		$this->db->where('type_of_leave', $type);
		$this->db->where('MONTH(datecreated)', $month); //For current month
		$this->db->where('staff_id', $staff_id);
		$res  = $this->db->get(db_prefix() . 'timesheets_requisition_leave')->result_array();
		$count  = count($res);
		//	echo '<pre>';print_r($res);
		if ($count > 0 or $res[0]['type_of_leave'] == 'saturday-leaves' or $res[0]['type_of_leave'] == 'half-days' or $res[0]['type_of_leave'] == 'short-leaves') {
			foreach ($res as $response) {
				//print_r($response['type_of_leave']);
				if (($response['type_of_leave'] == 'saturday-leaves') or ($response['type_of_leave'] == 'half-days') or ($response['type_of_leave'] == 'short-leaves')) {
					//echo $count;
					if ($count > 0) {
						echo 'hide';
						die;
				...
				} else if ($response['type_of_leave'] == 'planned_leaves' or $response['type_of_leave'] == 'unplanned_leaves' or $response['type_of_leave'] == 'unpaid-half-days') {
					echo 'hide';
					die;
				}
			}
		...
		/*foreach($res as $response){
				...
				$type_of_leave = $response['type_of_leave'];
				
					if($type_of_leave == $type){
						echo $type_of_leave;
						return true;
						}
					else{
	...
	public function reload_timesheets_byfilter()
	{
		...
		} else if (!$department && !is_admin()) {
			$arrdepartment = $this->staff_model->get_staff_based_on_department();
			$temp = '';
			foreach ($arrdepartment as $value) {
	...
	public function add_requisition_ajax()
	{
		...
		if ($this->input->post()) {
			...
			if (isset($data['number_of_days'])) {
				...
				$number_of_leaving_day = $data['number_of_days'];
			}
	...
	public function approve_comment()
	{
		...
		if ($this->input->post()) {
			...
			//$results = $this->timesheets_model->update_approval_status($data);

			$result = $this->timesheets_model->update_approval_comment($data);
	...
	public function reject_comment()
	{
		...
		if ($this->input->post()) {
			...
			$result = $this->timesheets_model->update_reject_comment($data);
			$this->load->library('session');
			$this->session->set_flashdata('message', 'Data has been added scussessfully');
			//$this->session->set_userdata('message', 'Data has been added scussessfully');
	...
	public function requisition_detail($id)
	{
		...
		$staff_id = $data['request_leave']->staff_id;
	...
	public function add_allocate_shiftwork($id = '')
	{
		...
		foreach ($data_ts as $ts) {
			...
			if (count($ts_type) <= 1) {
				...
				//now it will not show hours worked on table but only the attendence type
				// $staff_info['ts'] = $ts['type'] ;
			} else {
			...
			if (is_numeric($re)) {
				...
				if ($re == true) {
					...
					if ($type == 1) {
						...
						if ($data['edit_date']) {
							$staff_id_for_leave = $data['staff_id'];
							$staff_checkin_datetime = $data['edit_date'];

							// print_r($data);
							list($datePart, $timePart) = explode(" ", $staff_checkin_datetime);

							$datePart = date_create_from_format('m/d/Y', $datePart);

							$formattedDate = date_format($datePart, 'Y-m-d');
							// $datePart .= ' 00:00:00';

							// $checkLeaveQuery = "SELECT * FROM tbltimesheets_requisition_leave WHERE staff_id = ? ";


							$checkLeaveQuery = "SELECT * FROM tbltimesheets_requisition_leave WHERE staff_id = ? AND start_time = ?";
							$staff_present = $this->db->query($checkLeaveQuery, [$staff_id_for_leave, $formattedDate]);
							// $staff_present = $this->db->query($checkLeaveQuery,[$staff_id_for_leave]);

							// echo '<pre>';
							// print_r($staff_present->result_array()); 
							// echo '</pre>'; 
							// die;
							if ($staff_present->num_rows() == 1) {
								$updateTheLeaveQuery = "UPDATE tbltimesheets_requisition_leave SET number_of_leaving_day = number_of_leaving_day - 1 WHERE staff_id = ? AND start_time = ? AND number_of_leaving_day > 0";
								$this->db->query($updateTheLeaveQuery, [$staff_id_for_leave, $formattedDate]);
							}
						}


						$currentUserStaffid = get_staff()->staffid;
						// running update query when the check in is successful 
						// $sqlCurrentCheckIn = "SELECT * FROM tblcheck_in_out WHERE type_check = 1 AND DATE(date) = DATE(CURDATE()) AND staff_id = ? ";

						$sqlCheckIn = "SELECT * FROM tblcheck_in_out WHERE type_check = 1  AND staff_id = ? ORDER BY date DESC LIMIT 1";

						$sqlCheckOut = "SELECT * FROM tblcheck_in_out WHERE type_check = 2  AND staff_id = ? ORDER BY date DESC LIMIT 1";

						// $CurrentCheckInData = $this->db->query($sqlCurrentCheckIn,[$currentUserStaffid]);
						$checkInData = $this->db->query($sqlCheckIn, [$currentUserStaffid]);
						$lastCheckOutData = $this->db->query($sqlCheckOut, [$currentUserStaffid]);

						// $currentCheckinData = $CurrentCheckInData->result_array(); 
						...
						$interval = $curentCheckinTime->diff($lastCheckoutTime);

						$hour_diff = $interval->h + ($interval->days * 24);
						...
						// print_r($lastCheckoutTime); die;
						// echo " the difference in days is " . $days_diff . " days and " . $hour_diff; die;
						// $startTimeLeave = $prevTime->add(new DateInterval("P{$days_diff}D")); // Create a new DateTime object with added days
						// $start_time = $startTimeLeave->format('Y-m-d H:i:s');

						// $end_days_diff = $days_diff - 1;
						// $endTimeLeave = $latestTime->sub(new DateInterval("P{$end_days_diff}D")); // Create a new DateTime object with added days
						// $end_time = $endTimeLeave->format('Y-m-d H:i:s');


						// $endTimeLeave = $latestTime->sub(new DateInterval("P{$days_diff}D")); // Create a new DateTime object with added days
						// $end_time = $endTimeLeave->format('Y-m-d H:i:s');

						$checkInTime = $curentCheckinTime->format('H:i:s');
						$checkoutTime = $lastCheckoutTime->format('H:i:s');

						$queryForshiftTime = "SELECT shiftstart,shiftend FROM tblstaff_info WHERE staffid = ?";

						$staffShiftTime = $this->db->query($queryForshiftTime, [$currentUserStaffid]);
						$staffShiftTimeArray = $staffShiftTime->result_array();

						$checkInTime = strtotime($checkInTime);
						$shiftStartTime = strtotime($staffShiftTimeArray[0]['shiftstart']);
						$shiftEndTime = strtotime($staffShiftTimeArray[0]['shiftend']);

						// print_r($staffShiftTimeArray);

						// echo strtotime($checkInTime) . " " . strtotime($staffShiftTimeArray[0]['shiftstart']) . " " . strtotime($staffShiftTimeArray[0]['shiftend']) ; die;
						//echo "SELECT * FROM tbltimesheets_requisition_leave WHERE type_of_leave = 'present-leaves' OR type_of_leave = 'planned_leaves' OR type_of_leave = 'saturday-leaves' OR type_of_leave = 'holiday-leaves'  AND staff_id = $currentUserStaffid";die;
						...
						if ($start_time == $curr_date or $end_time == $curr_date) {
							redirect(admin_url());
						} else {
							if ($hour_diff > 15 && ($lastCheckoutTime->format('l') !== 'Sunday')) {
								// $updateQuery = "UPDATE tbltimesheets_requisition_leave SET number_of_leaving_day = number_of_leaving_day + 1 WHERE staff_id = ? ORDER BY start_time DESC LIMIT 1" ;

								// echo 'inside if'; die;
								$oneDayInterval = new DateInterval("P1D");
								// $end_time = $curentCheckinTime->sub($oneDayInterval);
								// $end_time_formatted = $end_time->format('Y-m-d');
								// $start_time = $lastCheckoutTime->add($oneDayInterval);
								$start_time = $lastCheckoutTime;
								// $start_time_formatted = $start_time->format('Y-m-d');

								// $sundays = intval($days_diff / 7) + ($start_time->format('N') + $days_diff % 7 >= 7);

								$updateQuery = "INSERT INTO tbltimesheets_requisition_leave( staff_id , subject, start_time, end_time, status, number_of_leaving_day) VALUES(?,?,?,?,?,?)";

								$markAbsentQuery = "INSERT INTO tbltimesheets_timesheet (staff_id , date_work , value , type, add_from) VALUES(?,?,?,?,?)";
								// $number_of_leaves = $days_diff - $sundays;

								log_message('error', $days_diff);

								for ($i = 0; $i < $days_diff; $i++) {
									$leavedate = $start_time->add($oneDayInterval)->format('Y-m-d');
									...
									$this->db->query($updateQuery, [$currentUserStaffid, "Staff did not checked in ", $leavedate, $leavedate, 4, 1]);
									$this->db->query($markAbsentQuery, [$currentUserStaffid, $leavedate, 0, 'AB', $currentUserStaffid]);
								}
						...
						//						$hours_worked_today = $this->timesheets_model->get_hour_check_in_out_staff(get_staff_user_id(), date("Y-m-d"));
						...
						//							]);
						//						} elseif ($hours_worked_today >= 5 && $hours_worked_today < 9) {
						//
						//							$this->db->insert('tbltimesheets_requisition_leave', [
						//								'staff_id' => get_staff_user_id(),
						//								'subject' => 'Half day',
	...
	public function timekeeping_data()
	{
		...
		$type_valid = ['AL', 'W', 'U', 'HO', 'E', 'L', 'B', 'SI', 'M', 'ME', 'NS', 'P', 'AB', 'HD'];
	...
	public function check_employee_attendance()
	{
		...
		$staffs_under_manager = (is_admin()) ? 178 : get_staff_user_id();
		...
		$data['staffs_in_select_option'] =  $this->staff_model->get('', ['team_manage' => $reporting_manager_id, 'active' => 1]);
		$staffs = $this->staff_model->get('', ['team_manage' => $staffs_under_manager, 'active' => 1]);
		...
		$selectedMonth = $this->input->post('month_year') ? $this->input->post('month_year') : $this->session->userdata('selectedMonth');
