 ...
 */
 ...
 function data_tables_init_project($aColumns, $sIndexColumn, $sTable, $join = [], $where = [], $additionalSelect = [], $sGroupBy = '', $searchAs = [])
 {
     $CI          = & get_instance();
                 ...
                 if ($search_value != '') {
                     ...
                     if (count($additionalSelect) > 0) {
                         foreach ($additionalSelect as $searchAdditionalField) {
                             ...
                             $sWhere .= 'convert(' . preg_replace('/\s+as\s+\w+\s*$/', '', $searchAdditionalField) . ' USING utf8)' . " LIKE '" . $CI->db->escape_like_str($search_value) . "%' ESCAPE '!' OR ";
                             // $sWhere .= 'convert(' .  $searchAdditionalField .  ' USING utf8)' . " LIKE '" . $CI->db->escape_like_str($search_value) . "%' ESCAPE '!' OR ";
                         }
                     }
                     $searchFound++;
