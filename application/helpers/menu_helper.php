...
function app_init_admin_sidebar_menu_items()
{
    ...
    // changing the default timesheet report link (by default it will display their own timesheet)
    ...
    // if (attendance_permission()) {
    //     $CI->app_menu->add_sidebar_menu_item('performance-review', [
    //         'name' => 'PEDMA',
    //         'href' => admin_url('staff/pedma_admin'),
    //         'icon' => 'fa-solid fa-chart-simple',
    //         'position' => 50,
    ...
    // display Interview button to those that are added as interviewer when interview is scheduled

    if (check_staff_is_interviewer() || is_admin() || in_the_department('HR')) {
        $CI->app_menu->add_sidebar_menu_item('Interviews', [
            'name' => 'Interviews',
            'href' => admin_url('recruitment/view_interviews'),
            'position' => 3,
            'icon' => 'fa fa-user',
            'badge' => [],
        ]);
    ...
    $CI->app_menu->add_sidebar_menu_item('Work', [
        'collapse' => true,
        // 'name'     => _l('als_reports'),
        'name' => "Work",
        'icon' => 'fa fa-laptop',
