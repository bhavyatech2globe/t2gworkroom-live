...

{
    ...
    public function get_staff_based_on_department()
    {
        $this->load->model('departments_model');
        $all_staff_data = $this->db->query('select * from tblstaff inner join tblstaff_departments on tblstaff.staffid = tblstaff_departments.staffid where tblstaff.active = 1;')->result();
    ...
    function carryForward($staffid, $doj, $month, $year)
    {
        ...
        $totalLeavesPreviousMonth = $this->noOfMonthsFromTo($doj,$prevDate) * $leaveRate;
    ...
    function monthlyLeaveBalance($staffid, $doj, $month, $year)
    {
        ...
        $leaveRate = $this->calculateEarnedLeaves($doj);
        $lastDayOfTheMonth = "$year-$monthFormatted-31 00:00:00";
