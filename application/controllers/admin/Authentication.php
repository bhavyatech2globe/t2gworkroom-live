...

{
...

    {
        ...
        // added check pass changed condition in the if block
        if ($this->input->post() ) {
    ...
    /**
     * This function will check if the staff is loggin in for the first time
     */
    public function check_pass_changed(){
        $staffid = get_staff_user_id();
       
        $staff_name = get_staff_full_name($staffid);
        $check_password_query = "SELECT * FROM tblactivity_log WHERE description LIKE '%User Reseted Password%' AND description LIKE '%$staffid%'";

        $changed_password = $this->db->query($check_password_query)->result_array();

        return $changed_password ? true : false;
