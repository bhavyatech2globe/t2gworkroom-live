...
class Projects extends AdminController
{
    ...
    public function view($id)
    {
        if (staff_can('view', 'projects') || $this->projects_model->is_member($id)) {
            ...
            // instead of getting all staff we are getting it based on department
            // $data['staff']   = $this->staff_model->get('', ['active' => 1]);
            if(is_admin()){
                $data['staff']   = $this->staff_model->get('', ['active' => 1]);
            } else {
            ...
            // $data['members'] = $this->staff_model->_get_staff_based_on_department();
            // foreach ($data['members'] as $key => $member) {
            //     $data['members'][$key]['total_logged_time'] = 0;
            //     $member_timesheets                          = $this->tasks_model->get_unique_member_logged_task_ids($member['staffid'], ' AND task_id IN (SELECT id FROM ' . db_prefix() . 'tasks WHERE rel_type="project" AND rel_id="' . $this->db->escape_str($id) . '")');

            //     foreach ($member_timesheets as $member_task) {
            //         $data['members'][$key]['total_logged_time'] += $this->tasks_model->calc_task_total_time($member_task->task_id, ' AND staff_id=' . $member['staffid']);
            //     }
            // }
            $data['bodyclass'] = '';
