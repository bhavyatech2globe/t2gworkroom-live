...
if ($v && strpos($v->version, '5.7') !== false) {
    ...
    $staffIdSelect = 'DISTINCT ANY_VALUE(staff_id) as staff_id';
    ...
    $staffIdSelect = 'DISTINCT staff_id';
...
// joining departments table

$join = [
    'LEFT JOIN ' . db_prefix() . 'tasks ON ' . db_prefix() . 'tasks.id = ' . db_prefix() . 'taskstimers.task_id',
    'LEFT JOIN '. db_prefix() .'staff_departments ON '. db_prefix() .'staff_departments.staffid = '. db_prefix() .'taskstimers.staff_id'
];
...
$department_where_query = '';

if(!is_admin()){
    $CI = & get_instance();
    $CI->load->model('departments_model');
    $depart_arr = $CI->departments_model->get_staff_departments(get_staff_user_id(),true);
    $depart_str = implode(',',$depart_arr);
    // print_r($depart); die;
    $department_where_query = ' AND departmentid IN (' . $depart_str . ')';

}

array_push($where,$department_where_query);
...
//change in query to only display the department inner joining with department table

$department_query = '';
if(!is_admin()){
    $department_query = 'LEFT JOIN '. db_prefix() .'staff_departments ON '.db_prefix().'staff_departments.staffid = '.db_prefix().'taskstimers.staff_id ';
}
$chartData = $this->ci->db->query('SELECT end_time - start_time logged_time_h,
end_time - start_time logged_time_d,start_time,end_time FROM ' . db_prefix() . 'taskstimers LEFT JOIN ' . db_prefix() . 'tasks ON ' . db_prefix() . 'tasks.id = ' . db_prefix() . 'taskstimers.task_id '.$department_query.'WHERE ' . trim($chartWhere))->result_array();
